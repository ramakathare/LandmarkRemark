...
export class CcaHttp {
    ...
    constructor( @Inject(PLATFORM_ID) private platformId, http: Http, protected transferState: TransferState) {
        this._http = http;
    }
    ...
    private async _requestCoreAsync(url: string, method: string, data: any, options: RequestOptions,
        ...
        try {
            ...
            cachedData = this.getFromCache(key);
            if (!cachedData) {
                ...
                response = Promise.resolve(cachedData);
